...
function SortTable() {
  ...
  for (var i = 0, len = rows.length; i < len; i++) {
    ...
    if (type == "D") {
      ...
      arrayOfRows[i].value = GetDateAndTimeSortingKey(dateAndTimeFormat, celltext);
    } else {
...
function GetDateAndTimeSortingKey(format, text) {
  if (format.length < 1) {
    return "";
  }
    ...
    const headers = [
      {
        ...
        href: 'javascript:SortTable(0,"S");',
      },
      { text: "Opaque ID", href: 'javascript:SortTable(1,"T");' },
      { text: "Score", href: 'javascript:SortTable(2,"N");' },
      {
        text: "Submitted At",
        href: 'javascript:SortTable(3,"D", "mdyhtsp");',
      },
    ];
