window.addEventListener('DOMContentLoaded', () => {
  ...
  const clearCookie = () => { 
      ...
      const newCookie = 'ccid=; max-age: 0; expires=Thu, 01 Jan 1970 00:00:00 GMT'
  ...
  document.getElementById('switchID').appendChild(createButton('hc-command', 'Switch to this ID', () => { 
      ...
      if (/^[a-z]{4}(-[a-z]{4}){3}$/.test(newccid)) {
        const newCookie = 'ccid=' + newccid + '; max-age: 0; expires=' + (new Date(new Date() * 1 + 180 * 24 * 60 * 60 * 1000)).toUTCString()
        document.cookie = newCookie 
        location = location                
      } else {
